...
#define totalQs 4 // total queues for MLFQ scheduler
#define NULL 0
#define S 512

// MLFQ data structures
int maxTicks[totalQs] = {2,4,16,32};        // # clock ticks allowed for a given queue by index
int allowedQuanta[totalQs] = {1,2,4,8};   // # clock ticks for round robin in a given queue by index
...
struct {
  int whichScheduler;             // Determines which scheduler to use: 0 = original round robin, 1 = random, 2 = MLFQ
  struct spinlock lock;
  struct proc proc[NPROC];
  // MLFQ invarients
  int totalCurrentTicks;          // # of timer interupts <= S, should only be updated by CPU 0
  struct proc* queues[totalQs];   // heads of doubly-linked lists of RUNNABLE or RUNNING processes for each queue
} ptable;
...
// Decides which scheduler to use: 0 = original round robin, 1 = random, 2 = MLFQ
int changeScheduler() {
    int schedulerOption;
    acquire(&ptable.lock);
    ptable.whichScheduler = (ptable.whichScheduler + 1) % 3;
    schedulerOption = ptable.whichScheduler;
    release(&ptable.lock);
...
struct proc* getRandomRunnableProccess() {
  struct proc* p;
  struct proc* runnableProccess[NPROC];
  uint totalRunnableProcesses = 0;
  uint maxIndex;
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
    if(p->state == RUNNABLE) {
      maxIndex = totalRunnableProcesses;
  ...
  if (totalRunnableProcesses == 0) {
    ...
    uint randomIndex = getRandomIndex(maxIndex);
...
scheduler(void)
{
  for(;;){
    ...
    // Disable interupts while picking a new scheduler
    cli();
  }
}
...
void roundRobbinScheduler() {
  ...
  for(;;){
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      ...
      if (ptable.whichScheduler != 0) {
        // change to a different scheduler if scheduler option doesn't equal the RR option (0)
        release(&ptable.lock);
        return;
      }
    }
...
void randomScheduler() {
  ...
  for(;;){
    ...
    if (ptable.whichScheduler != 1) {
      // change to a different scheduler if scheduler option doesn't equal the Random option (1)
      release(&ptable.lock);
      return;
    }
    release(&ptable.lock);
...
void MLFQscheduler() {
  ...
  for (;;) {
    ...
    if (ptable.whichScheduler != 2) {
      // change to a different scheduler if scheduler option doesn't equal the MLFQ option (2)
      release(&ptable.lock);
      return;
    }
...
// disconnect p from current queue, patching said queue, and reset proc variables
// NOTE: ptable lock must be held**
