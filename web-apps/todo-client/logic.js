BASE_URL = "https://afternoon-brushlands-28790.herokuapp.com/"
...
function listTODOs(item_created) {
    ...
    fetch(BASE_URL + 'todos', {credentials: 'include'} ).then(function(response) {
...
function retrieveTODO(ID) {
    fetch(BASE_URL + 'todos/' + ID.toString(), {credentials: 'include'} ).then(function(response) {
        return response.json();
    });
}
...
function createTODO(title) {
    ...
    var default_todo = "short_description=" + title + "&long_description= &priority=1&desired_completion_date= &due_date= &completion_status=FALSE&date_entered=" + date_entered;
...
function replaceTODO(ID, encoded_string) {
    fetch(BASE_URL + 'todos/' + ID.toString(), {
        method: 'PUT',
        credentials: 'include',
        headers: {
            'Content-Type': 'application/x-www-form-urlencoded'
...
function createAuth(userOrSession) {
    ...
    fetch(BASE_URL + userOrSession, {
        ...
        if (response.status == 401) {
            ...
            reg_error.innerHTML = "Email already taken, or invalid.";
        } 
