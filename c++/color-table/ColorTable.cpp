...
void ColorTable::insertStripes( const Color& color, const size_t& posistion1, const size_t& posistion2) {
	size_t start = position1;
	while (	start < position2) {
		mColors[start] = color;
		Color inverted_color = color;
		inverted_color.invert();
		mColors[start+1] = inverted_color;
		start += 1	
	}
}
